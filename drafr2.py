...
while True:
    ...
    if frame_count % detect_interval == 0:
        ...
        for r in results:
            if r.boxes:
                last_hand_box = max(r.boxes, key=lambda b: (b.xyxy[0][2] - b.xyxy[0][0]) * (b.xyxy[0][3] - b.xyxy[0][1]))
    else:
        results = None  # no new detection

    if last_hand_box is not None:
        x1, y1, x2, y2 = map(int, last_hand_box.xyxy[0])
        x_center = (x1 + x2) // 2
        y_center = (y1 + y2) // 2
        box_w = x2 - x1
        box_h = y2 - y1
